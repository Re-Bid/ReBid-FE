...
const DetailAdminApprove = ({ bidId }) => {
  ...
  const onValid = (data) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_BASE_URL}/admin/bids/${bidId}/confirm/reservation`,
        {
          startDate: startDate,
          endDate: endDate,
        },
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-4 ">
        <div className="">입찰 시작 날짜</div>
        <div className=" space-x-2">
          <input
            {...register("startDate")}
            type="date"
            className="border border-borderColor p-1 rounded-lg placeholder:text-borderColor "
          />
          <input
            {...register("startTime")}
            type="time"
